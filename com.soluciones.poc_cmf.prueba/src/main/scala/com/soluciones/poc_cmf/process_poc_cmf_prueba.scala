...
import com.soluciones.poc_cmf.product._ 
import com.soluciones.poc_cmf.institucion._
...
object process_poc_cmf_prueba {
  ...
  */
  def main(args : Array[String]) {
    //Creacion API
    val huemulBigDataGov  = new huemul_BigDataGovernance(s"Masterizacion tabla tbl_poc_cmf_prueba - ${this.getClass.getSimpleName}", args, globalSettings.Global)
  ...
  def process_master(huemulBigDataGov: huemul_BigDataGovernance, ControlParent: huemul_Control, param_ano: Integer, param_mes: Integer): huemul_Control = {
    ...
    try {             
      ...
      val tbl_poc_cmf_product_mes = huemulBigDataGov.DF_ExecuteQuery("df_ins",s"""select * from ${tbl_poc_cmf_product_mes.getTable()}​​""").collect()
      

      val tbl_poc_cmf_institucion_mes= new tbl_instituciones(huemulBigDataGov, Control)
      val tbl_poc_cmf_institucion_mes = huemulBigDataGov.DF_ExecuteQuery("df_ins",s"""select * from ${​​tbl_instituciones.getTable()}​​""").collect()

      val tbl_poc_cmf_activosbanco2_mes = new tbl_productos(huemulBigDataGov, Control)
      ...
      huemulTable.DF_from_SQL("FinalRAW"
                          , s""" SELECT p.TBL_POC_CMF_PRODUCT_MES.PRODUCTO_ID as producto_id, i.[TBL_POC_CMF_INSTITUCION_MES.INSTITUCION_ID] as institucion_id,
                                ...
                                INNER JOIN tbl_comun_producto_data as p ON ab2.TBL_POC_CMF_ACTIVOSBANCO2_MES.PRODUCTO=p.TBL_POC_CMF_PRODUCT_MES.PRODUCT_DESC
                                INNER JOIN tbl_instituciones_data as i ON ab2.TBL_POC_CMF_ACTIVOSBANCO2_MES.INSTITUCIONES = i.TBL_POC_CMF_INSTITUCION_MES.INSTITUCION_DESC;
                              """)
