...
object process_activosbanco{
  ...
  */
  def main(args : Array[String]) {
    //Creacion API
    val huemulBigDataGov  = new huemul_BigDataGovernance(s"Masterizacion tabla tbl_poc_cmf_activosbanco - ${this.getClass.getSimpleName}", args, globalSettings.Global)
  ...
  def process_master(huemulBigDataGov: huemul_BigDataGovernance, ControlParent: huemul_Control, param_ano: Integer, param_mes: Integer): huemul_Control = {
    ...
    try {             
      ...
      if (!DF_RAW_AC2.open("DF_RAW_AC2", Control, param_ano, param_mes, 1, 0, 0, 0))       
        Control.RaiseError(s"error encontrado, abortar: ${DF_RAW_AC2.Error.ControlError_Message}")
      ...
      val huemulTable = new tbl_poc_cmf_activosbanco(huemulBigDataGov, Control)
      ...
      huemulTable.DF_from_SQL("FinalRAW"
                          , s"""SELECT TO_DATE("$param_ano-$param_mes-1") as periodo_mes
                               ...
                               FROM DF_RAW_AC1 activos INNER JOIN  ${tabla_instituciones.getTable()} insti
                               INNER JOIN ${tabla_productos.getTable()} product
                               where activos.Instituciones = insti.institucion_desc 
                               AND activos.producto = product.prod_Path
                                UNION ALL
                                SELECT TO_DATE("$param_ano-$param_mes-1") as periodo_mes                            
                                   
                                    ,insti2.institucion_id as institucion_id
                                    ,product2.producto_id
                                    ,activos2.Monto

                              FROM DF_RAW_AC2 activos2 INNER JOIN  ${tabla_instituciones.getTable()} insti2
                               INNER JOIN ${tabla_productos.getTable()} product2
                               where activos2.Instituciones = insti2.institucion_desc 
                               AND activos2.producto = product2.prod_Path
